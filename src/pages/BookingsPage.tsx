...
const bookingLabels = { flight: 'Flight', lodging: 'Stay', car: 'Car', activity: 'Plan', restaurant: 'Table', transfer: 'Ride' }
  ...
  return (
    <>
        ...
        {bookings.map((booking) => {
          return (
            <article className="booking-card" key={booking.id}>
              <span className="booking-kind">{bookingLabels[booking.kind]}</span>
              ...
              <div className="booking-action">
                ...
                {booking.sourceUrl && <a className="button secondary" href={booking.sourceUrl} target="_blank" rel="noopener noreferrer">Open source</a>}
      ...
      <section className="lodging-section">
        ...
        <div className="lodging-grid">{lodging.map((hotel) => <article className="booking-card lodging-card" key={hotel.id}><span className="booking-kind">Stay</span><div><StatusBadge status={hotel.status} /><h3>{hotel.name}</h3><address>{hotel.address}</address><p>{hotel.nights} {hotel.nights === 1 ? 'night' : 'nights'} · {hotel.roomStyle}</p><p className="quote-note"><strong>Live quote · checked {hotel.quoteCheckedOn}</strong>{hotel.quoteDisplay}</p><dl className="lodging-facts"><div><dt>Taxes and fees</dt><dd>{hotel.taxesAndFees}</dd></div><div><dt>Cancellation</dt><dd>{hotel.cancellationTerms}</dd></div><div><dt>Payment</dt><dd>{hotel.paymentTerms}</dd></div><div><dt>Check-in</dt><dd>{hotel.checkInConstraints}</dd></div><div><dt>Parking</dt><dd>{hotel.parkingPlan}</dd></div><div><dt>Stairs and bags</dt><dd>{hotel.stairsAndLuggage}</dd></div></dl><ul>{hotel.notes.map((note) => <li key={note}>{note}</li>)}</ul></div><div className="booking-action"><span><small>live quote · for two</small><strong>€{hotel.totalEstimateEur}</strong><small>€{hotel.nightlyEquivalentEur.toFixed(2)} per night</small></span><a className="button secondary" href={hotel.mapsUrl} target="_blank" rel="noopener noreferrer">Open map</a><a className="button secondary" href={hotel.bookingUrl} target="_blank" rel="noopener noreferrer">Refresh quote</a></div></article>)}</div>
      </section>
      ...
      <section className="car-rental-card">
        ...
        <div className="truth-note"><p><strong>Prime Visa plan:</strong> request Chase’s proof-of-coverage letter, confirm the supplier permits declining optional CDW/LDW, and remember that card cover does not replace third-party liability or the rental hold.</p></div>
