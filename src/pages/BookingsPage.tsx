...
export function BookingsPage() {
  ...
  return (
    <>
      <header className="page-intro"><p className="eyebrow">Nothing implied, everything actionable</p><h1>Bookings: the runway.</h1><p>Only the flights are confirmed. Every hotel, car and activity below remains a quote, target or request until you reserve it.</p></header>
      ...
      <section className="lodging-section">
        <div className="section-heading"><div><p className="eyebrow">12 continuous nights</p><h2>Where you’ll sleep</h2></div><p>All amounts are working estimates for two, including the known taxes captured during research.</p></div>
        <div className="lodging-grid">{lodging.map((hotel) => <article className="booking-card lodging-card" key={hotel.id}><span className="booking-icon"><Hotel /></span><div><StatusBadge status={hotel.status} /><h3>{hotel.name}</h3><p>{hotel.city} · {hotel.nights} {hotel.nights === 1 ? 'night' : 'nights'} · {hotel.roomStyle}</p><ul>{hotel.notes.map((note) => <li key={note}>{note}</li>)}</ul><small>{hotel.parkingPlan}</small></div><div className="booking-action"><span><small>estimate for two</small><strong>€{hotel.totalEstimateEur}</strong></span><a className="button secondary" href={hotel.bookingUrl} target="_blank" rel="noopener noreferrer">Refresh quote</a></div></article>)}</div>
      </section>
      ...
      <section className="car-rental-card">
        ...
        <dl><div><dt>Base quote</dt><dd>€{trip.car.basePriceEur}</dd></div><div><dt>Hard allocation</dt><dd>€{trip.car.protectedBudgetEur}</dd></div><div><dt>Refundable hold</dt><dd>€{trip.car.depositEur}</dd></div><div><dt>Excess</dt><dd>€{trip.car.excessEur}</dd></div><div><dt>Fuel</dt><dd>{trip.car.fuelPolicy}</dd></div><div><dt>Mileage</dt><dd>{trip.car.mileage}</dd></div></dl>
